...
int main(int argc, char **argv) {
    // HARDCODED FOR EXP 01: IMMEDIATE (APPROACH 2)
    int experiment_id = 1;                  
    const char* system_name = "Lab";        
    const char* approach_name = "Immediate"; 
    int config_id = 3;                      // Set to 1, 2, or 3
    
    NX = 1000;              
    NY = 400;               
    NUM_Points = 1e6;       // Change to 10^2, 10^4, 10^6, 10^8, 10^9 for scaling
    Maxiter = 10;           
    int num_threads = 1;    // Exp 1 is Serial
    ...
    for (int iter = 0; iter < Maxiter; iter++) {
        ...
        if (num_threads == 1) deleted_count = mover_immediate_serial(points, dx, dy);
        else deleted_count = mover_immediate_parallel(points, dx, dy);
    ...
    const char* filename = "exp1_scaling_results.csv";
